...
export default function CatalogoPublico() {
  ...
  const visibles = useMemo(() => {
    ...
    return productos.filter((p) => [p.nombre, p.categoria, ...(p.variantes || []).flatMap((v) => [v.nombre, v.codigo, ...(v.atributos || []).map((a) => `${a.atributo} ${a.valor}`)])].join(" ").toLowerCase().includes(term));
  }, [productos, q]);
    ...
    <main className="min-h-screen bg-[#faf9f6] text-stone-950">
      ...
      <header className="sticky top-0 z-20 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-5 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden flex-1 items-center gap-4 overflow-x-auto text-sm text-stone-700 lg:flex">
            {categorias.map((c) => <span key={c} className="whitespace-nowrap rounded-md px-2 py-1 hover:bg-stone-100">{c}</span>)}
          </nav>
      ...
      <section className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-[2rem] bg-stone-900 text-white shadow-sm">
          ...
          <div className="absolute left-8 top-1/2 max-w-lg -translate-y-1/2 sm:left-12">
            ...
            <button className="mt-6 rounded-full bg-white px-6 py-3 text-sm font-bold text-stone-950">Descubrir</button>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-10 sm:px-6 lg:px-8">
        <div className="mb-5 flex items-end justify-between gap-3">
          <div><h2 className="text-2xl font-black text-stone-950">Productos en stock</h2><p className="mt-1 text-sm text-stone-500">Precios y disponibilidad actualizados desde Odoo.</p></div>
