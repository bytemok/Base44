...
function NuevaVenta() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMsg(Number(sena.monto) > 0 ? `Venta ${res.data.name} creada en borrador. Reserva registrada por Mercado Pago.` : `Venta ${res.data.name} creada en borrador.`); setItems([]); setSena({ monto: "", metodo: "", operacion: "" });
    } catch (e) {
