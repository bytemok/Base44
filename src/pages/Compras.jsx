...
import SelectPopover from "@/components/erp/SelectPopover";
                  ...
                  <tr key={r.product_id} className={procesando.has(r.product_id) ? "opacity-50 bg-amber-50" : selected.has(r.product_id) ? "bg-slate-50" : "hover:bg-slate-50"}>
                    ...
                    <td className="px-3 py-2.5">
                      <SelectPopover
                        ...
                        onChange={(v) => setSupSel((m) => ({ ...m, [r.product_id]: v === "" ? null : Number(v) }))}
                        options={[
                          { value: "", label: r.proveedor || "Sin proveedor" },
                          ...(proveedores || []).map((p) => ({ value: p.id, label: p.nombre })),
                        ]}
                        triggerClassName="w-40 px-2 py-1 text-xs"
                      />
                    </td>
