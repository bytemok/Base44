...
export default function VentasTable({ rows, onOpen, compactDelivered = false, showDebt = true }) {
  if (compactDelivered) {
    return (
      <div className="max-h-[72vh] overflow-auto rounded-b-md border-x border-b border-slate-200 bg-white shadow-sm">
        <table className="w-full border-separate border-spacing-0 text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((r, i) => (
              <tr key={r.db_id || r.id || i} onClick={() => onOpen?.(r.db_id)} className={`${onOpen ? "cursor-pointer" : ""} bg-white hover:bg-violet-50/40`}>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <tr key={r.db_id || r.id || i} onClick={() => onOpen?.(r.db_id)} className={`${onOpen ? "cursor-pointer" : ""} bg-white hover:bg-violet-50/40`}>
                ...
                {showDebt && <td className={`px-3 py-2 whitespace-nowrap text-right font-semibold ${r.adeudado > 0 ? "text-red-600" : "text-emerald-600"}`}>{fmt.format(r.adeudado || 0)}</td>}
